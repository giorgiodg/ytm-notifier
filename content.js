...
function stripYear(str) {
  return str.replace(/\s*•\s*\d{4}$/, "");
}

function getSongInfo() {
  const strategies = [
    // Attempt 1: Player bar
    () => {
      ...
      if (playerBar) {
        ...
        const rawArtist = playerBar
          .querySelector('.subtitle, .secondary, [class*="artist"]')
          ?.textContent?.trim();

        artist = stripYear(rawArtist);

        const thumbEl =
          playerBar.querySelector("img") ||
          playerBar.querySelector("yt-img-shadow img") ||
...
function checkSongChange() {
  ...
  if (
    ...
    if (Notification.permission === "granted") {
      ...
      const iconUrl =
        songInfo.thumbnail && songInfo.thumbnail.startsWith("http")
          ? songInfo.thumbnail
          : musicIconDataUrl;
